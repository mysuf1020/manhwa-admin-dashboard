	...
	<div class="bg-slate-900 border border-slate-800 rounded-xl p-4 md:p-5 mb-8 shadow-lg">
		{#if user}
			<form action="?/addComment" method="POST" use:enhance={submitEnhance} class="flex gap-3">
				...
				<div class="grow">
					<textarea name="content" rows="3" required placeholder="Wah, adegannya seru banget... (Tulis diskusi Anda)" class="w-full bg-slate-950 border border-slate-700 rounded-lg p-4 text-slate-300 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all resize-none shadow-inner"></textarea>
					<div class="flex justify-end mt-3">
						<button type="submit" class="bg-purple-600 hover:bg-purple-500 text-white font-semibold py-2 px-6 rounded-lg transition-transform active:scale-95 shadow-lg shadow-purple-900/20 flex items-center gap-2">
							Kirim Posting
		...
		{:else}
			<div class="py-8 text-center bg-slate-950/50 rounded-lg border border-dashed border-slate-800">
				<p class="text-slate-400 mb-4 text-sm">Anda harus login untuk ikut berdiskusi.</p>
				<a href="/login" class="inline-block bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 font-semibold py-2 px-6 rounded-lg transition-colors">Login via Akun</a>
			</div>
		{/if}
	</div>
	...
	<div class="flex flex-col gap-5">
		{#each getTopLevel() as comment (comment.id)}
			<div class="flex gap-3 md:gap-4">
				<div class="w-10 h-10 rounded-full flex shrink-0 items-center justify-center font-bold text-sm uppercase {comment.user.role === 'admin' ? 'bg-rose-900/50 text-rose-400 border border-rose-500/20' : 'bg-slate-800 text-slate-300 border border-slate-700'}">
					{comment.user.username.slice(0, 2)}
				</div>
				<div class="grow min-w-0">
					<div class="bg-slate-900 border border-slate-800 p-4 rounded-xl rounded-tl-sm">
						...
						<p class="text-slate-400 text-sm md:text-[15px] leading-relaxed whitespace-pre-wrap">{comment.content}</p>
					</div>
					{#if user}
					...
					{#if replyingTo === comment.id && user}
						<form action="?/addComment" method="POST" use:enhance={submitEnhance} class="mt-2 ml-2">
							...
							<textarea name="content" rows="2" required placeholder="Tulis balasan..." class="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-sm text-slate-300 focus:outline-none focus:border-purple-500 resize-none"></textarea>
							<button type="submit" class="mt-1.5 bg-purple-600/80 hover:bg-purple-500 text-white text-xs font-semibold py-1.5 px-4 rounded-lg transition-colors">Kirim Balasan</button>
						</form>
					{/if}

					<!-- Replies -->
					{#each getReplies(comment.id) as reply (reply.id)}
						<div class="flex gap-3 mt-3 ml-2 pl-3 border-l-2 border-slate-800">
							<div class="w-7 h-7 rounded-full flex shrink-0 items-center justify-center font-bold text-[10px] uppercase {reply.user.role === 'admin' ? 'bg-rose-900/50 text-rose-400' : 'bg-slate-800 text-slate-400'}">
								{reply.user.username.slice(0, 2)}
							</div>
							<div class="grow min-w-0">
								<div class="flex items-center gap-2 mb-0.5 flex-wrap">
									<span class="font-bold text-slate-300 text-sm">{reply.user.username}</span>
								...
								<p class="text-slate-400 text-sm leading-relaxed whitespace-pre-wrap">{reply.content}</p>
							</div>
						</div>
					{/each}
				</div>
