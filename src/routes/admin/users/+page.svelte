...
<div class="mb-6">
	<h2 class="text-2xl font-bold text-white mb-1">Manajemen User</h2>
	<p class="text-sm text-slate-400">Total: {data.users.length} pengguna terdaftar</p>
</div>
...
<div class="bg-slate-900 rounded-xl border border-slate-800 shadow-xl overflow-hidden">
	<table class="w-full text-left">
		<thead class="bg-slate-950/50 text-slate-400 text-xs uppercase border-b border-slate-800">
			<tr>
				...
				<tr class="hover:bg-slate-800/50 transition-colors">
					<td class="p-4">
						<div class="flex items-center gap-3">
							...
							{:else}
								<div class="w-8 h-8 rounded-full bg-purple-600 flex items-center justify-center text-sm font-bold text-white">{user.username[0].toUpperCase()}</div>
							{/if}
							<div>
					...
					<td class="p-4 text-sm text-slate-400">{timeAgo(String(user.createdAt))}</td>
					<td class="p-4 text-right">
						<form method="POST" action="?/setRole" use:enhance class="inline-flex gap-1">
							<input type="hidden" name="userId" value={user.id}>
							<select name="role" class="bg-slate-950 border border-slate-700 rounded-lg px-2 py-1 text-xs text-white appearance-none">
