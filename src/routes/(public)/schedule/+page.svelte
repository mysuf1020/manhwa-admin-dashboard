...
<div class="max-w-7xl mx-auto px-4 py-8 md:py-12">
	<div class="text-center mb-10">
		...
		<p class="text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">Update komik favoritmu setiap harinya. Jadwal bisa berubah sewaktu-waktu tergantung dari author atau tim penerjemah.</p>
	</div>

	<!-- Day Tabs -->
	<div class="flex overflow-x-auto no-scrollbar gap-2 mb-8 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-2 rounded-2xl snap-x">
		{#each data.days as day, i (day)}
			<button
				onclick={() => (activeTab = day)}
				class="flex-1 shrink-0 snap-center px-4 md:px-6 py-3 rounded-xl font-bold transition-all text-sm md:text-base border {activeTab === day ? 'bg-purple-600 text-slate-900 dark:text-white border-purple-500 shadow-lg shadow-purple-900/40' : 'bg-transparent text-slate-600 dark:text-slate-400 border-transparent hover:bg-slate-100 dark:bg-slate-800 hover:text-slate-800 dark:text-slate-200'} {i === currentDayMapping && activeTab !== day ? 'ring-1 ring-purple-500/50' : ''}"
			>
				{day}
	...
	<div class="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl min-h-[400px] p-4 md:p-6 shadow-xl relative overflow-hidden">
		...
		<div class="relative z-10">
			<h2 class="text-xl font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
				...
				<span class="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-xs px-2.5 py-1 rounded-full ml-auto">{data.schedule[activeTab].length} Komik</span>
			</h2>
			...
			{:else}
				<div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4">
					{#each data.schedule[activeTab] as comic (comic.id)}
						<a href="/comic/{comic.slug}" class="group block relative overflow-hidden rounded-xl bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 hover:border-purple-500/50 transition-all hover:shadow-[0_0_15px_rgba(168,85,247,0.15)]">
							<div class="aspect-3/4 w-full relative">
								...
								<div class="absolute top-2 left-2 flex gap-1 flex-col">
									<span class="text-[10px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider
										{comic.type === 'Manhwa' ? 'bg-orange-500/90 text-slate-900 dark:text-white' : ''}
										{comic.type === 'Manga' ? 'bg-blue-500/90 text-slate-900 dark:text-white' : ''}
										{comic.type === 'Manhua' ? 'bg-emerald-500/90 text-slate-900 dark:text-white' : ''}">
										{comic.type}
									</span>
								</div>
								<div class="absolute bottom-0 inset-x-0 p-3">
									<h3 class="text-sm font-bold text-slate-900 dark:text-white leading-tight line-clamp-2 group-hover:text-purple-400 transition-colors" title={comic.title}>{comic.title}</h3>
									<p class="text-[10px] text-slate-600 dark:text-slate-400 mt-1 line-clamp-1">{comic.genres || 'Unknown Genre'}</p>
								</div>
							</div>
						</a>
					{/each}
